...
export class DashboardService {
  ...
  getDashboardCount(): Observable<DashboardCounter> {
    return this.http
      .get(`${END_POINTS.restDashboard}/counters`)
      .pipe(map((resp: ResponseWrapper) => resp.data));
  }

  loadDashboardList(): Observable<DashboardCounter[]> {
    return this.http.get(`${END_POINTS.restDashboard}/list`)
        .pipe(map((resp: ResponseWrapper) => resp.data));
  }

  getDashboards(param: QueryParam): Observable<ResponseWrapper> {
    return this.http.get(`${END_POINTS.restDashboard}?page=${param.page}&pageSize=${param.pageSize}&orderby=${param.orderby}`)
