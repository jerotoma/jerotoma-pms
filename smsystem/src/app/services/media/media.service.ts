...
export class MediaService {
  ...
  getMedia(mediaId: number): Observable<Media> {
    return this.http
      .get(`${API_END_POINTS.uploads}/${mediaId}`)
      .pipe(map((resp: ResponseWrapper) => resp.data));
  }

  loadMedia(): Observable<Media[]> {
    return this.http.get(`${API_END_POINTS.uploads}`)
        .pipe(map((resp: ResponseWrapper) => resp.data));
  }
  ...
  getMediaPaginated(param: QueryParam): Observable<ResponseWrapper> {
    return this.http.get(`${API_END_POINTS.uploads}/paginated?page=${param.page}&pageSize=${param.pageSize}&orderby=${param.orderby}&userType=${param.userType}`)
  ...
  updateMedia(data?: any): Observable<Media> {
    return this.http.put(`${API_END_POINTS.uploads}`, data)
    .pipe(map((resp: ResponseWrapper) => resp.data));
  }

  search(data: any, param: QueryParam): Observable<ResponseWrapper> {
    return this.http.post(`${API_END_POINTS.uploads}/search?page=${param.page}&pageSize=${param.pageSize}&orderby=${param.orderby}&userType=${param.userType}`, data)
    .pipe(map((resp: ResponseWrapper) => resp));
  }
