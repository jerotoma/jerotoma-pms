...
export class PositionService {
  ...
  getPosition(positionId: number): Observable<any> {
    return this.http
      .get<any>(`${API_END_POINTS.positions}/${positionId}`)
      .pipe(retry(3), catchError(this.errorHandler));
  }

  loadPositionList(): Observable<Position[]> {
    return this.http.get<any>(
        `${API_END_POINTS.positions}/list`)
        .pipe(map((resp: ResponseWrapper) => resp.data ));
  }

  getPositions(param: QueryParam): Observable<HttpResponse<any> | HttpErrorResponse> {
    return this.http.get<any>(
        `${API_END_POINTS.positions}?page=${param.page}&pageSize=${param.pageSize}&orderby=${param.orderby}`,
        {observe: 'response'})
      .pipe(retry(3), catchError(this.errorHandler));
  }
  createPosition(data?: any): Observable<HttpResponse<any> | HttpErrorResponse> {
    return this.http.post<any>(`${API_END_POINTS.positions}`, data, {observe: 'response'});
  }

  deletePosition(positionId: number): Observable<HttpResponse<any> | HttpErrorResponse> {
    return this.http.delete<any>(`${API_END_POINTS.positions}/${positionId}`, {observe: 'response'});
  }

  updatePosition(data?: any): Observable<HttpResponse<any> | HttpErrorResponse> {
    return this.http.put<any>(`${API_END_POINTS.positions}`, data, {observe: 'response'});
  }
